...
const TYPE_LABEL = { shared: 'Shared', single: 'Single', double: 'Double', triple: 'Triple' }
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <p className="text-sm text-slate-500">{data?.rooms?.length || 0} rooms</p>
        <button
      ...
      {loading ? (
        ...
        <div className="card">
          <EmptyState message="No rooms yet — start by adding a room" />
        </div>
      ) : (
            ...
            return (
              <div key={r._id} className="card p-5">
                <div className="flex items-start justify-between">
                  <div>
                    <h3 className="text-lg font-extrabold text-slate-800">Room {r.number}</h3>
                    <p className="text-xs text-slate-400 mt-0.5">
                      {TYPE_LABEL[r.type] || r.type} {r.floor ? `• Floor ${r.floor}` : ''}
                      {r.floorName && ` • ${r.floorName}`}
                      {r.apartmentName && ` • Apt ${r.apartmentName}`}
                      {r.propertyName && ` • ${r.propertyName}`}
                    </p>
                  </div>
                  <Badge label={r.status === 'active' ? 'Active' : 'Inactive'} cls={r.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-600'} />
                </div>
