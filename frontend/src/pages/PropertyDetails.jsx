...
const BED_STYLE = {
  available: { chip: 'bg-emerald-50 border-emerald-300 hover:bg-emerald-100', dot: 'bg-emerald-500', label: 'متاح', text: 'text-emerald-700' },
  reserved: { chip: 'bg-amber-50 border-amber-300 hover:bg-amber-100', dot: 'bg-amber-500', label: 'محجوز', text: 'text-amber-700' },
  occupied: { chip: 'bg-primary-50 border-primary-300', dot: 'bg-primary-600', label: 'مشغول', text: 'text-primary-800' },
  maintenance: { chip: 'bg-slate-100 border-slate-300 hover:bg-slate-200', dot: 'bg-slate-500', label: 'صيانة', text: 'text-slate-600' },
}

const MINI = [
  { key: 'floors', icon: LayoutGrid, label: 'الأدوار', bg: 'bg-primary-50 text-primary-700', onlyHouse: true },
  { key: 'apartments', icon: Home, label: 'الشقق', bg: 'bg-amber-50 text-amber-600' },
  { key: 'rooms', icon: DoorOpen, label: 'الغرف', bg: 'bg-blue-50 text-blue-600' },
  { key: 'beds', icon: BedDouble, label: 'إجمالي الأسرة', bg: 'bg-slate-100 text-slate-700' },
  { key: 'available', icon: Sparkles, label: 'متاح', bg: 'bg-emerald-50 text-emerald-600' },
  { key: 'reserved', icon: CalendarClock, label: 'محجوز', bg: 'bg-amber-50 text-amber-600' },
  { key: 'occupied', icon: User, label: 'مشغول', bg: 'bg-primary-50 text-primary-700' },
  { key: 'maintenance', icon: Wrench, label: 'صيانة', bg: 'bg-slate-100 text-slate-600' },
]
  ...
  return (
    <div className={`p-3 rounded-xl border ${st.chip}`}>
      ...
      <div className="flex items-center justify-between mt-1">
        <span className="text-[11px] font-extrabold text-slate-700">{fmtMoney(bed.price)} ج.م</span>
        <div className="flex items-center gap-0.5">
          {canEdit && (
            <>
              <button className="p-1 rounded hover:bg-white/70 text-slate-500" title={bed.monthlyRent ? 'تغيير سعر السرير' : 'تحديد سعر السرير'} onClick={() => onPrice(bed)}>
                <Pencil size={10} />
              </button>
              {bed.status !== 'occupied' && (
                <button className="p-1 rounded hover:bg-red-100 text-red-500" title="حذف السرير" onClick={() => onDelete(bed)}>
                  <Trash2 size={10} />
                </button>
              )}
            </>
      ...
      {bed.summer && (
        <div className="mt-1.5 p-2 rounded-lg bg-orange-50 border border-orange-200">
          ...
          <p className="text-[10px] font-bold text-orange-700">{fmtMoney(bed.summer.rent)} ج.م/شهر {bed.summer.deposit > 0 && `• تأمين ${fmtMoney(bed.summer.deposit)}`}</p>
        </div>
      )}
      {bed.status === 'occupied' && bed.student && !bed.summer ? (
        ...
        <div className="flex items-center gap-1.5 mt-2">
          ...
          {bed.status === 'available' && canEdit && (
            <button className="btn-ghost text-emerald-700 !py-1 !px-1.5 text-[11px]" title="تعيين طالب" onClick={() => onTransfer(bed, true)}>
              <User size={12} /> تعيين
            </button>
          )}
        </div>
    ...
    <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          <p className="font-extrabold text-slate-800">غرفة {room.number}</p>
          <span className="text-[10px] font-bold text-slate-400">
            {room.capacity} سرير • {fmtMoney(room.monthlyRent)} ج.م
          </span>
        </div>
        <div className="flex items-center gap-1">
          {room.status === 'inactive' && <Badge label="متوقفة" cls="bg-slate-200 text-slate-600" />}
          {canEdit && (
            <>
              <button className="btn-ghost text-slate-500" title="تعديل الغرفة" onClick={() => onEditRoom(room)}>
                <Pencil size={13} />
              </button>
              <button className="btn-ghost text-red-600" title="حذف الغرفة" onClick={() => onDeleteRoom(room)}>
                <Trash2 size={13} />
              </button>
            </>
          )}
    ...
    <div className="p-4 rounded-2xl bg-white border border-slate-200 shadow-sm">
      <div className="flex flex-wrap items-center justify-between gap-2 mb-3">
        <div className="flex items-center gap-2">
          ...
          <p className="font-extrabold text-slate-800">شقة {ap.name}</p>
          {ap.code && <span className="font-mono text-[11px] text-slate-400" dir="ltr">{ap.code}</span>}
          {ap.status === 'inactive' && <Badge label="متوقفة" cls="bg-slate-200 text-slate-600" />}
          {ap.monthlyRent > 0 && <span className="text-[11px] font-bold text-slate-400">إيجار افتراضي {fmtMoney(ap.monthlyRent)} ج.م</span>}
        </div>
        <div className="flex items-center gap-1">
          {canEdit && (
            <>
              <button className="btn-ghost text-slate-500" title="مصروف صيانة" onClick={() => onAddMaintenance(ap)}>
                <Wrench size={14} />
              </button>
              <button className="btn-ghost text-slate-500" title="إضافة غرفة" onClick={() => onAddRoom(ap)}>
                <Plus size={14} /> غرفة
              </button>
              <button className="btn-ghost" title="تعديل الشقة" onClick={() => onEdit(ap)}>
                <Pencil size={13} />
              </button>
              <button className="btn-ghost text-red-600" title="حذف الشقة" onClick={() => onDelete(ap)}>
                <Trash2 size={13} />
              </button>
            </>
          )}
      ...
      {fin && ap.rooms.length > 0 && (
        <div className="flex flex-wrap gap-2 mb-3 text-[11px] font-bold">
          <span className="px-2.5 py-1 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200">متوقع {fmtMoney(fin.expected)} ج.م</span>
          <span className="px-2.5 py-1 rounded-lg bg-blue-50 text-blue-700 border border-blue-200">محصّل {fmtMoney(fin.paid)} ج.م</span>
          <span className="px-2.5 py-1 rounded-lg bg-red-50 text-red-600 border border-red-200">متبقي {fmtMoney(fin.remaining)} ج.م</span>
          <span className="px-2.5 py-1 rounded-lg bg-amber-50 text-amber-700 border border-amber-200">تأمين {fmtMoney(fin.deposits)} ج.م</span>
          <span className="px-2.5 py-1 rounded-lg bg-slate-50 text-slate-600 border border-slate-200">مشغول {fin.occupied} سرير</span>
          {fin.maintenance > 0 && <span className="px-2.5 py-1 rounded-lg bg-slate-100 text-slate-700 border border-slate-300">صيانة {fmtMoney(fin.maintenance)} ج.م</span>}
          <span className="px-2.5 py-1 rounded-lg bg-violet-50 text-violet-700 border border-violet-200">صافي {fmtMoney(fin.net)} ج.م</span>
        </div>
      )}
        ...
        <div className="flex flex-wrap gap-2 mb-3">
          {(maintenance || []).map((m) => (
            <span key={m._id} className="flex items-center gap-1.5 px-2.5 py-1 rounded-lg bg-slate-50 border border-slate-200 text-[11px] font-bold text-slate-600">
              ...
              {m.name} — {fmtMoney(m.amount)} ج.م <span className="text-slate-400 font-mono" dir="ltr">{m.date}</span>
              {canEdit && (
                <button className="text-red-500 hover:text-red-700" title="حذف المصروف" onClick={() => onDeleteMaintenance(m)}>
                  <Trash2 size={11} />
                </button>
              )}
            </span>
      ...
      {ap.rooms.length === 0 ? (
        <p className="text-xs text-slate-400 text-center py-3">لا توجد غرف في هذه الشقة</p>
      ) : (
...
export default function PropertyDetails() {
  ...
  const deleteRoom = (room) => del(`/properties/${id}/rooms/${room._id}`, `حذف غرفة ${room.number}؟`)

  const deleteLevel = (x) => {
    if (x.kind === 'floor') del(`/properties/${id}/floors/${x._id}`, `حذف الدور ${x.name}؟`)
    else if (x.kind === 'apartment' || (x.rooms && Array.isArray(x.rooms))) del(`/properties/${id}/apartments/${x._id}`, `حذف الشقة ${x.name}؟`)
    else deleteRoom(x)
  }
  ...
  const editBedPrice = async (bed) => {
    const v = window.prompt(
      `سعر السرير ${bed.bedNumber} في غرفة ${bed.roomNumber} شهريًا.\nاتركه فارغًا ليأخذ سعر الغرفة (${bed.roomMonthlyRent})`,
      bed.monthlyRent ?? ''
    )
    if (v === null) return
    try {
      await api.put(`/rooms/${bed.roomId}/beds/${bed.bedNumber}`, { monthlyRent: String(v).trim() === '' ? null : Number(v) })
      toast('تم تحديث سعر السرير')
      refetch()
    } catch (e) {
      toast(errMsg(e), 'error')
    }
  ...
  const deleteBed = async (bed) => {
    if (!window.confirm(`حذف السرير ${bed.bedNumber} من غرفة ${bed.roomNumber}؟`)) return
    try {
  ...
  const deleteMaintenance = async (m) => {
    if (!window.confirm(`حذف مصروف الصيانة "${m.name}" (${fmtMoney(m.amount)} ج.م)؟`)) return
    try {
    ...
    <div className="space-y-4">
      ...
      <div className="card p-6">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <h2 className="text-xl font-extrabold text-slate-800 flex items-center gap-2">
                {property.name}
                <Badge label={property.type === 'house' ? 'بيت كامل' : 'شقة مستقلة'} cls={property.type === 'house' ? 'bg-primary-100 text-primary-700' : 'bg-amber-100 text-amber-700'} />
                {GENDER[property.gender] && <Badge {...GENDER[property.gender]} />}
                {property.status === 'inactive' && <Badge label="متوقف" cls="bg-slate-200 text-slate-600" />}
              </h2>
        ...
        <div className="mt-4 grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-3">
          <div className="p-3 rounded-xl bg-emerald-50 border border-emerald-200">
            <p className="text-[10px] font-bold text-emerald-600">إيجار الشهر المتوقع</p>
            <p className="text-lg font-extrabold text-emerald-700 mt-0.5">{fmtMoney(totals?.expected ?? 0)} ج.م</p>
          </div>
          <div className="p-3 rounded-xl bg-blue-50 border border-blue-200">
            <p className="text-[10px] font-bold text-blue-600">المحصّل هذا الشهر</p>
            <p className="text-lg font-extrabold text-blue-700 mt-0.5">{fmtMoney(totals?.paid ?? 0)} ج.م</p>
          </div>
          <div className="p-3 rounded-xl bg-red-50 border border-red-200">
            <p className="text-[10px] font-bold text-red-600">المتبقي</p>
            <p className="text-lg font-extrabold text-red-700 mt-0.5">{fmtMoney(totals?.remaining ?? 0)} ج.م</p>
          </div>
          <div className="p-3 rounded-xl bg-amber-50 border border-amber-200">
            <p className="text-[10px] font-bold text-amber-600">إجمالي التأمين</p>
            <p className="text-lg font-extrabold text-amber-700 mt-0.5">{fmtMoney(totals?.deposits ?? 0)} ج.م</p>
          </div>
          <div className="p-3 rounded-xl bg-slate-100 border border-slate-300">
            <p className="text-[10px] font-bold text-slate-600">مصروفات الصيانة</p>
            <p className="text-lg font-extrabold text-slate-700 mt-0.5">{fmtMoney(totals?.maintenance ?? 0)} ج.م</p>
          </div>
          <div className="p-3 rounded-xl bg-violet-50 border border-violet-200">
            <p className="text-[10px] font-bold text-violet-600">الصافي</p>
            <p className="text-lg font-extrabold text-violet-700 mt-0.5">{fmtMoney(totals?.net ?? 0)} ج.م</p>
          </div>
        </div>
        ...
        <div className="mt-4 flex items-center gap-3 text-xs">
          ...
          <span className="font-bold text-slate-600">نسبة الإشغال {stats.occupancy}%</span>
        </div>
      </div>
      ...
      {property.type === 'house' && (
        <div className="space-y-4">
          {floors.length === 0 && (
            <div className="card">
              <EmptyState message="لا توجد أدوار بعد — أضف الدور الأول" />
            </div>
          )}
          {floors.map((f) => (
            <div key={f._id} className="card overflow-hidden">
              <div className="flex flex-wrap items-center justify-between gap-2 px-5 py-3 bg-slate-950 text-white">
                <div className="flex items-center gap-2">
                  ...
                  <p className="font-extrabold">{f.name || 'دور بدون اسم'}</p>
                  {f.code && <span className="font-mono text-[11px] text-slate-400" dir="ltr">{f.code}</span>}
                  {f.status === 'inactive' && <Badge label="متوقف" cls="bg-white/10 text-slate-300" />}
                </div>
                <div className="flex items-center gap-1">
                  {canEdit && (
                    <>
                      <button className="p-1.5 rounded-lg hover:bg-white/10 text-slate-300" title="تعديل الدور" onClick={() => setLevel({ kind: 'floor', item: f })}>
                        <Pencil size={14} />
                      </button>
                      <button className="p-1.5 rounded-lg hover:bg-red-500/20 text-red-400" title="حذف الدور" onClick={() => deleteLevel({ kind: 'floor', ...f })}>
                        <Trash2 size={14} />
                      </button>
                    </>
                  )}
              ...
              <div className="p-4 space-y-3">
                {f.apartments.length === 0 ? (
                  <p className="text-xs text-slate-400 text-center py-3">لا توجد شقق في هذا الدور</p>
                ) : (
                  f.apartments.map((ap) => (
                    <ApartmentBlock
                      key={ap._id}
          ...
          {unlinkedApartments.length > 0 && (
            <div className="card p-4">
              <p className="font-extrabold text-slate-700 text-sm mb-3">شقق بدون دور</p>
              <div className="space-y-3">
          ...
          {unlinkedRooms.length > 0 && (
            <div className="card p-4">
              <p className="font-extrabold text-slate-700 text-sm mb-3">غرف بدون شقة</p>
              <div className="space-y-3">
                {unlinkedRooms.map((r) => (
