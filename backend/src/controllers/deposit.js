...
function compute(student) {
  ...
  return {
    ...
    status,
    statusLabel: status === 'full' ? REFUND_STATUS.full : status === 'partial' ? REFUND_STATUS.partial : status === 'pending' ? REFUND_STATUS.pending : status === 'paid' ? PAYMENT_STATUS.paid : PAYMENT_STATUS.unpaid,
    receipts: d.receipts,
    audit: d.audit,
  };
}
